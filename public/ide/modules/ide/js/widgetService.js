...
ideServices.service('WidgetService', ['ProjectService', 'Type', 'ResourceService', 'CanvasService', 'FontMesureService', function (ProjectService,
    ...
    //MyContacts
    fabric.MyContacts = fabric.util.createClass(fabric.Object, {
        ...
        initialize: function (level, options) {
            ...
            this.textName = info.textName;
            this.textNum = info.textNum;
            this.nameOffsetX = info.nameOffsetX;
            this.nameOffsetY = info.nameOffsetY;
            this.numOffsetX = info.numOffsetX;
            this.numOffsetY = info.numOffsetY;
            this.fontFamily = level.info.fontFamily;
            this.fontSize = level.info.fontSize;
            this.fontColor = level.info.fontColor;
            this.fontBold = level.info.fontBold;
        ...
        _render: function (ctx) {
            try {
                ...
                if (this.textName) {
                    ...
                    ctx.font = fontString;
                    ctx.textAlign = 'left';
                    ctx.textBaseline = 'top';//使文本垂直居中
                    ctx.fillText(this.textName, -(this.width / 2) + this.nameOffsetX, -(this.height / 2) + this.nameOffsetY);
                    ctx.restore();
                }
                if (this.textNum) {
                    ctx.save();
                    ...
                    ctx.textAlign = 'left';
                    ctx.textBaseline = 'top';//使文本垂直居中
                    ctx.fillText(this.textNum, -(this.width / 2) + this.numOffsetX, -(this.height / 2) + this.numOffsetY);
                    ctx.restore();
                }
            }
            catch (err) {
                console.log('错误描述', err);
                toastr.warning('渲染MyContacts出错');
            }
        }
    });
    fabric.MyContacts.fromLevel = function (level, callback, option) {
        callback && callback(new fabric.MyContacts(level, option));
    };
    fabric.MyContacts.prototype.toObject = (function (toObject) {
        return function () {
            return fabric.util.object.extend(toObject.call(this), {
                currentColor: this.currentColor,
                currentImageElement: this.currentImageElement
    ...
    })(fabric.MyContacts.prototype.toObject);
    fabric.MyContacts.fromObject = function (object, callback) {
        var level = ProjectService.getLevelById(object.id);
        callback && callback(new fabric.MyContacts(level, object));
    };
    fabric.MyContacts.async = true;
